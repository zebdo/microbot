...
public class EnsouledHeadSlayerScript extends Script {
	...
	*/
        ...
        var enemy = Rs2Npc.getNpcs("Reanimated").collect(Collectors.toList());
        if (enemy.isEmpty()) {
            ...
            if (spell != null) {
                ...
                sleepUntil(Rs2Combat::inCombat, 30000);
            }
        } else {
            Microbot.log("Found NPC");
            Rs2Npc.attack(enemy.stream().findFirst().get());
        }
    }
    ...
    private boolean getRunesAndEnsouledHeads(EnsouledHeadSlayerConfig config) {
        if (config.ensouledHeads().getName().equals("All")) {
            ...
            int availableSlots = Rs2Inventory.emptySlotCount() - config.foodAmount() - 3;
            ...
            if (config.useArcheusLibraryTeleport() && Rs2Inventory.count("earth rune") < 2 && Rs2Inventory.count("law rune") < 1) {
                Rs2Bank.withdrawX("earth rune", Rs2Random.between(2,10));
                Rs2Bank.withdrawX("law rune", Rs2Random.between(1,10));
            }
