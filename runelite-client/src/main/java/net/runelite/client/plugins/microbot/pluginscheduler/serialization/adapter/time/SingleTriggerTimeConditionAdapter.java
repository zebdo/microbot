...
public class SingleTriggerTimeConditionAdapter implements JsonSerializer<SingleTriggerTimeCondition>, JsonDeserializer<SingleTriggerTimeCondition> {
    ...
    public JsonElement serialize(SingleTriggerTimeCondition src, Type typeOfSrc, JsonSerializationContext context) {
        ...
        json.addProperty("targetTime", targetUtc.toLocalTime().format(TIME_FORMAT));
        json.addProperty("targetDate", targetUtc.toLocalDate().format(DATE_FORMAT));
    ...
    public SingleTriggerTimeCondition deserialize(JsonElement json, Type typeOfT, JsonDeserializationContext context) 
        ...
        try {
            ...
            // Get target time
              ...
              // Parse time values
            LocalTime serializedStartTime = LocalTime.parse(jsonObject.get("targetTime").getAsString(), TIME_FORMAT);                            
              // Parse date values
            LocalDate serializedStartDate = LocalDate.parse(jsonObject.get("targetDate").getAsString(), DATE_FORMAT);
