...
public class LightPuzzleSolver {
    ...
    public void solve()
    {
        ...
        Tile[][] sceneTiles = Microbot.getClient().getScene().getTiles()[Microbot.getClient().getPlane()];
        Point tl = findStartTile(sceneTiles);
        if (tl == null)
        {
            Microbot.log("Failed to locate start of light puzzle");
            return;
        }
    ...
    {
        if (e.getMessage().startsWith("completed!"))
        {
            solved = true;
        }
    }
