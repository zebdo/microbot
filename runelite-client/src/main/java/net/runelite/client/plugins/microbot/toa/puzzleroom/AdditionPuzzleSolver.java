...
public class AdditionPuzzleSolver {
    ...
    public void solve()
    {
        ...
        Tile[][] sceneTiles = Microbot.getClient().getTopLevelWorldView().getScene().getTiles()[Microbot.getClient().getTopLevelWorldView().getPlane()];
        Point tl = findStartTile(sceneTiles);
        if (tl == null)
        {
            Microbot.log("Failed to locate start of addition puzzle");
            return;
        }
