...
public class GotrPrePostScheduleRequirements extends PrePostScheduleRequirements{
    ...
    protected boolean initializeRequirements() {      
        ...
        int[] gotrWorlds = { 409, 445, 464, 478, 522, 534 }; // GOTR worlds
        
        // Use LocationRequirementUtil to select world with most players from GOTR worlds
        int mostPopulatedGotrWorld = LocationRequirementUtil.getMostPopulatedWorldFromList(gotrWorlds);
        int selectedWorld = mostPopulatedGotrWorld != -1 ? mostPopulatedGotrWorld : 522; // Fallback to 522
        
        this.register(new LocationRequirement(BankLocation.GUARDIANS_OF_THE_RIFT, 15, true, selectedWorld, ScheduleContext.PRE_SCHEDULE, RequirementPriority.MANDATORY));             
        ...
        ItemRequirementCollection.registerRunecraftingOutfit(this, RequirementPriority.RECOMMENDED, ScheduleContext.PRE_SCHEDULE);
        
        // Graceful outfit - weight reduction and run energy restoration
        ItemRequirementCollection.registerGracefulOutfit(this, RequirementPriority.RECOMMENDED, ScheduleContext.PRE_SCHEDULE);
        ...
        ItemRequirementCollection.registerProspectorOutfit(this, RequirementPriority.RECOMMENDED,6, ScheduleContext.PRE_SCHEDULE);
        ...
        this.register(ItemRequirement.createOrRequirement(
            ...
            EquipmentInventorySlot.CAPE, -1, RequirementPriority.RECOMMENDED, 10, "Runecrafting cape (any variant)", ScheduleContext.PRE_SCHEDULE,
            Skill.RUNECRAFT, 99, null, null, false
        ));
        this.register(ItemRequirement.createOrRequirement(
            Arrays.asList(ItemID.SKILLCAPE_MINING, ItemID.SKILLCAPE_MINING_TRIMMED), 1,
            EquipmentInventorySlot.CAPE, -1, RequirementPriority.RECOMMENDED, 4, "Mining cape (any variant)", ScheduleContext.PRE_SCHEDULE,
        ...
        this.register(new ItemRequirement(
            ...
            EquipmentInventorySlot.AMULET, RequirementPriority.RECOMMENDED, 8, "Binding necklace (essential for combination runes)", ScheduleContext.PRE_SCHEDULE
        ));
        ItemRequirementCollection.registerAmuletOfGlory(this, RequirementPriority.RECOMMENDED, 6, ScheduleContext.PRE_SCHEDULE,true);              
        ...
        this.register(ItemRequirement.createOrRequirement(
            ...
            EquipmentInventorySlot.BODY, RequirementPriority.RECOMMENDED, 8, "Varrock armour (any variant, for mining fragments)", ScheduleContext.PRE_SCHEDULE
        ));
        
        // RING - Ring of endurance excellent for run energy, then teleport rings
        this.register(new ItemRequirement(
            ItemID.RING_OF_ENDURANCE,
            EquipmentInventorySlot.RING, RequirementPriority.RECOMMENDED, 8, "Ring of endurance (for run energy)", ScheduleContext.PRE_SCHEDULE
        ));
        
        // Alternative boots for weight reduction
        this.register(ItemRequirement.createOrRequirement(
            Arrays.asList(ItemID.IKOV_BOOTSOFLIGHTNESS, ItemID.IKOV_BOOTSOFLIGHTNESSWORN),
            EquipmentInventorySlot.BOOTS, RequirementPriority.RECOMMENDED, 4, "Boots of lightness (for weight reduction)", ScheduleContext.PRE_SCHEDULE
        ));
        // ====================================================================
        ...
        this.register(new LocationRequirement(BankLocation.GRAND_EXCHANGE,
            20,
                true, -1,ScheduleContext.POST_SCHEDULE, RequirementPriority.MANDATORY));
